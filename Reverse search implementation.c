...
int main()
    {
        ...
        int i,j,k,l,m;
        int count = 0;
        printf("Please enter the node number to which paths are to be traced:");
        scanf("%d", &last_node);
        printf("\nProcessing..");
        for (m=0; m<max_row ; m++)
            {
        ...
        do
        {
            ...
            for (l=0; l<max_row;l++)
                {
                    if (array_read[l][2] == ntbe)
                        {
                            LIST[array_read[l][1] - 1] = 1;
                            Final_data[array_read[l][1] - 1][1] = 1;
                            Final_data[array_read[l][2] - 1][2] = array_read[l][1] - 1;
                        }
                }
            LIST[k-1] = 0;
            count = 0;
